...
save_dir="/vepfs/fs_ckps/xufanjie/UniNMR/finetune/${dataset}/5cv/${selected_atom}_${weight_name}_global_${11}_kener_${12}_${arch}_${loss}_lr_${lr}_bs_${global_batch_size}_${warmup}_${epoch}"
if [ -d "${save_dir}" ]; then
    rm -rf ${save_dir}
...
for fold in $(seq 0 $(($nfolds - 1)))
    ...
    python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) $data_path --user-dir /vepfs/fs_users/xufanjie/Uni_NMR/Uni-Mol/uninmr  --train-subset train --valid-subset valid \
        ...
        --selected-atom $selected_atom  --split-mode cross_valid --nfolds $nfolds --fold $fold --cv-seed $cv_seed $GLOBAL_DISTANCE_FLAG $GAUSS_FLAG 
...
python /vepfs/fs_users/xufanjie/Uni_NMR/Uni-Mol/uninmr/utils/get_result.py --path $save_dir  --mode cv  2>&1 | tee ${save_dir}/result.log