dataset='lmdb_limit_primitive_false_rcut6'
data_path="/vepfs/fs_users/xufanjie/data/CSD/${dataset}" # replace to your data path
dict_name='/vepfs/fs_ckps/xufanjie/UniNMR/dict/oc_limit_dict.txt'
n_gpu=8
MASTER_PORT=33333
lr=1e-4
wd=1e-4
...
python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) $data_path --user-dir /vepfs/fs_users/xufanjie/Uni_NMR/Uni-Mol/uninmr --train-subset train --valid-subset valid \
       ...
       --save-interval-updates 20000 --validate-interval-updates 20000 --keep-interval-updates 10 --no-epoch-checkpoints  \
       ...
       --finetune-from-model $pretrain_weight  --dict-name $dict_name --gaussian-kernel --save-dir $save_dir \
